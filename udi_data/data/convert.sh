...
# 体外诊断试剂分类目录
sed -i 's/,"I-/,"Ⅰ-/g' 体外诊断试剂分类目录.csv
sed -i 's/,"I"$/,Ⅰ/g' 体外诊断试剂分类目录.csv
awk -F ',' 'NR>4 {gsub(/^"|"$/, "", $2); split($2, lb, " "); print "medical_device_category_6840-" lb[1] "," lb[2] ",6840-" lb[1] ",medical_device_category_6840,,,," substr(lb[1], 1, 1)}' 体外诊断试剂分类目录.csv | uniq >> medical.device.category.csv
awk -v FPAT='([^,]+)|("[^"]+")' 'NR>4 {gsub(/^"|"$/, "", $1); gsub(/^"|"$/, "", $2); split($2, lb, " "); print "medical_device_category_6840-" $1 "," $3 ",6840-" $1 ",medical_device_category_6840-" lb[1] ",," $4 ",," $5}' 体外诊断试剂分类目录.csv >> medical.device.category.csv
