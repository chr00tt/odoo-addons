...
# 体外诊断试剂分类目录
awk -v FPAT='([^,]*)|("[^"]*")' -v OFS=, 'NR>1 {print "medical_device_category_6840-" $2 "," $3 ",6840-" $2 ",medical_device_category_6840,,,," $7}' 体外诊断试剂分类目录.csv | uniq >> medical.device.category.csv
awk -v FPAT='([^,]*)|("[^"]*")' -v OFS=, 'NR>1 {print "medical_device_category_6840-" $2 "-" $4 "," $5 ",6840-" $2 "-" $4 ",medical_device_category_6840-" $2 ",," $6 ",," $7}' 体外诊断试剂分类目录.csv >> medical.device.category.csv
