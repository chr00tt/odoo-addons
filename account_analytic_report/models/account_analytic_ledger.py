...
class AnalyticLedgerCustomHandler(models.AbstractModel):
    ...
    def _get_aml_values(self, options, analytic_account_ids, offset=0, limit=None):
        rslt = {analytic_account_id: [] for analytic_account_id in analytic_account_ids}

        analytic_account_ids_wo_none = [x for x in analytic_account_ids if x]
        directly_linked_aml_analytic_account_clauses = []
        directly_linked_aml_analytic_account_params = []
        indirectly_linked_aml_analytic_account_params = []
        indirectly_linked_aml_analytic_account_clause = 'aml_with_analytic_account.analytic_account_id IS NOT NULL'
        if None in analytic_account_ids:
            directly_linked_aml_analytic_account_clauses.append('account_move_line.analytic_account_id IS NULL')
        if analytic_account_ids_wo_none:
            directly_linked_aml_analytic_account_clauses.append('account_move_line.analytic_account_id IN %s')
            directly_linked_aml_analytic_account_params.append(tuple(analytic_account_ids_wo_none))
            indirectly_linked_aml_analytic_account_clause = 'aml_with_analytic_account.analytic_account_id IN %s'
            indirectly_linked_aml_analytic_account_params.append(tuple(analytic_account_ids_wo_none))
        directly_linked_aml_analytic_account_clause = '(' + ' OR '.join(directly_linked_aml_analytic_account_clauses) + ')'
        ...
        for column_group_key, group_options in report._split_options_per_column_group(options).items():
            ...
            # For the move lines directly linked to this analytic_account
            queries.append(f'''
                SELECT
                ...
                LEFT JOIN res_analytic_account analytic_account               ON analytic_account.id = account_move_line.analytic_account_id
                LEFT JOIN account_account account           ON account.id = account_move_line.account_id
                LEFT JOIN account_journal journal           ON journal.id = account_move_line.journal_id
                WHERE {where_clause} AND {directly_linked_aml_analytic_account_clause}
                ORDER BY account_move_line.date, account_move_line.id
            ''')

            # For the move lines linked to no analytic_account, but reconciled with this analytic_account. They will appear in grey in the report
            queries.append(f'''
                SELECT
                    account_move_line.id,
                    account_move_line.date,
                    ...
                    account_move_line.account_id,
                    account_move_line.payment_id,
                    aml_with_analytic_account.analytic_account_id,
                    account_move_line.currency_id,
                    account_move_line.amount_currency,
                    account_move_line.matching_number,
                    CASE WHEN aml_with_analytic_account.balance > 0 THEN 0 ELSE ROUND(
                        partial.amount * currency_table.rate, currency_table.precision
                    ) END                                                                               AS debit, 
                    CASE WHEN aml_with_analytic_account.balance < 0 THEN 0 ELSE ROUND(
                        partial.amount * currency_table.rate, currency_table.precision
                    ) END                                                                               AS credit, 
                    - sign(aml_with_analytic_account.balance) * ROUND(
                        partial.amount * currency_table.rate, currency_table.precision
                    )                                                                                   AS balance, 
                    account_move.name                                                                   AS move_name,
                    account_move.move_type                                                              AS move_type,
                ...
                FROM {tables}
                    ...
                    account_move,
                    account_move_line aml_with_analytic_account,
                    account_journal journal,
                    account_account account
                WHERE
                    (account_move_line.id = partial.debit_move_id OR account_move_line.id = partial.credit_move_id)
                    AND account_move_line.analytic_account_id IS NULL
                    AND account_move.id = account_move_line.move_id
                    AND (aml_with_analytic_account.id = partial.debit_move_id OR aml_with_analytic_account.id = partial.credit_move_id)
                    AND {indirectly_linked_aml_analytic_account_clause}
        ...
        for aml_result in self._cr.dictfetchall():
            if aml_result['key'] == 'indirectly_linked_aml':
                ...
                # Append the line to the analytic_account found through the reconciliation.
                if aml_result['analytic_account_id'] in rslt:
                    rslt[aml_result['analytic_account_id']].append(aml_result)

                # Balance it with an additional line in the Unknown analytic_account section but having reversed amounts.
                if None in rslt:
                    rslt[None].append({
                        **aml_result,
