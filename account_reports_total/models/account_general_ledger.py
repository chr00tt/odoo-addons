...
class GeneralLedgerCustomHandler(models.AbstractModel):
    ...
    def _add_period_total_lines(self, line_dict_id, groupby, options, progress, offset, unfold_all_batch_data, new_line, balance, lines,bln_last_one=False):
        ...
        if progress_date:
            # 本日合计
            if options['daily_total'] and (line_date != progress_date or bln_last_one):
                total_line = self._get_daily_total_line(report, line_dict_id, options, progress, balance)
                lines.append(total_line)
            # 本月合计
            if options['monthly_total'] and (line_date[:7] != progress_date[:7] or bln_last_one):
                total_line = self._get_monthly_total_line(report, line_dict_id, options, progress, balance)
                lines.append(total_line)
            # 本年累计
            if options['yearly_total'] and (line_date[:4] != progress_date[:4] or bln_last_one):
